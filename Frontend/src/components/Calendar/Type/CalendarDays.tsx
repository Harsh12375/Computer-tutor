					...
					<div className='calendar-grid'>
						{
								Array.from({ length: numOfDays }, (_, dayIndex) => {
									const colDate = dateObj(dayIndex);
									const isToday = colDate.format('YYYY-MM-DD') === dayjs().format('YYYY-MM-DD');
									return (
										<div
											key={`time-col-${dayIndex + 1}`}
											className={`calendar__date-cols ${isToday ? 'calendar-column--today' : ''}`}
											ref={dayColumnEl}
										>
											<TimeBlockCol
												dayIndex={dayIndex}
